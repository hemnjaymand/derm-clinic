...
class S3CompatibleStorage implements StorageProvider {
  ...
  async upload({ buffer, fileName, contentType, folder }: {
    ...
    try {
      const key = `${folder}/${crypto.randomUUID()}-${sanitizeFileName(fileName)}`;
...
/** ترکیب هر دو — برای فیلدهایی که همزمان تصویر و ویدیو قبول می‌کنند (مثلاً Gallery) */
export const ALLOWED_MEDIA_TYPES = [...ALLOWED_IMAGE_TYPES, ...ALLOWED_VIDEO_TYPES];

export const MAX_IMAGE_SIZE_BYTES = 5 * 1024 * 1024; // 5MB — برای آیکون، لوگو، بنر
export const MAX_VIDEO_SIZE_BYTES = 50 * 1024 * 1024; // 50MB — برای ویدیوهای تستیمونیال

/** سقف حجم مجاز را بر اساس نوع فایل واقعی تعیین می‌کند */
export function getMaxSizeForType(contentType: string): number {
  return ALLOWED_VIDEO_TYPES.includes(contentType) ? MAX_VIDEO_SIZE_BYTES : MAX_IMAGE_SIZE_BYTES;
}